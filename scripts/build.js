        ...
        setup(build) {
            ...
            build.onLoad({ filter: /\.svelte$/ }, async (args) => {
                ...
                for (const warning of compiled.warnings) {
                    if (warning.code?.startsWith('a11y_') || warning.code === 'state_referenced_locally' || warning.code === 'css_unused_selector') continue
...
console.log('🔨 开始构建 svelte-mobile-ui...\n')
...
await esbuild.build({
    ...commonOptions,
    format: 'esm',
    outfile: path.join(DIST, 'svelte-mobile-ui.mjs'),
    splitting: false,
})
...
await esbuild.build({
    ...commonOptions,
    format: 'cjs',
    outfile: path.join(DIST, 'svelte-mobile-ui.cjs'),
})
...
await esbuild.build({
    ...commonOptions,
    format: 'iife',
    globalName: 'SvelteMobileUI',
    outfile: path.join(DIST, 'svelte-mobile-ui.iife.js'),
    external: [],
})
